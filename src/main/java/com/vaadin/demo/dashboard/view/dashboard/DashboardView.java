...
public final class DashboardView extends Panel implements View,
	...
	private Button buildDownloadTestBtn() {
		final Button downloadTestBtn = new Button();
		downloadTestBtn.setDescription("Download test file");
		downloadTestBtn.setCaption("Download Test");
		downloadTestBtn.setIcon(FontAwesome.DOWNLOAD);

		FileDownloader templateDownloader = null;
		String testFile = DummyDataProvider.getCsvDataFileName();
		if( testFile instanceof String )
		{
			File templateFile = new File(testFile);

			if( templateFile.exists())
			{
				FileResource templateResource = new FileResource(templateFile);

				templateDownloader = new FileDownloader(templateResource);
				templateDownloader.extend(downloadTestBtn);
				downloadTestBtn.setVisible(true);
			}
		}

		if( templateDownloader == null )
		{
			downloadTestBtn.addClickListener(e->{
				Notification.show("Test download",
						"No test file to download - call support.",
						Type.ERROR_MESSAGE );});
		}

		return downloadTestBtn;
	}
    ...
    private Component buildHeader() {
        ...
        BrowserWindowOpener window = new BrowserWindowOpener("http://www.vaadin.com");
        Button vaadin = new Button("Vaadin Website");
        window.extend(vaadin);
        HorizontalLayout tools = new HorizontalLayout(buildDownloadTestBtn(),
        		notificationsButton, edit, vaadin );
